---
title: "Nava — OS-Native Sound Engine in Python"
type: project
status: active
year: 2023
tags: [python, audio, cross-platform, open-source]
domain: speech
---

**Role:** Lead developer
**Org:** [[../industry-experience/open-science-laboratory|Open Science Laboratory (OpenSciLab)]]
**Platforms:** Windows, macOS, Linux
**Dependencies:** None
**Adoption:** 30k+ downloads. Recognized on the **Real Python podcast** as one of the host's
favorite libraries of the week.
**Funding:** [[../honors-and-awards/psf-grant-nava|PSF Software Development Grant — Nava]] — PSF grant for OS-based sound engines (Mar 2025).

## Summary
Nava plays sound in Python with no dependencies and no platform restrictions, on Windows,
macOS, and Linux. That is the whole library. The case for it is that "just play this file" gets
complicated once you want it to work everywhere. Nava is an OS-native sound engine, and it stays
lightweight and easy to drop into a program.

## Links
[GitHub](https://github.com/openscilab/nava/) · [PyPI](http://pepy.tech/project/nava)

## Related
[[sharif-wav2vec2|Sharif-Wav2Vec2.0 — Farsi Speech Recognition Model]] — the other audio-domain work.
